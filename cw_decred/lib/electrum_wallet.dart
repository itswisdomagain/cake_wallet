...
import 'package:cw_decred/address_to_output_script.dart';
...
import 'package:cw_decred/electrum_transaction_history.dart';
import 'package:cw_decred/decred_transaction_no_inputs_exception.dart';
import 'package:cw_decred/decred_transaction_priority.dart';
import 'package:cw_decred/decred_transaction_wrong_balance_exception.dart';
import 'package:cw_decred/decred_unspent.dart';
import 'package:cw_decred/decred_wallet_keys.dart';
import 'package:cw_decred/file.dart';
import 'package:cw_decred/pending_decred_transaction.dart';
import 'package:cw_decred/script_hash.dart';
...
abstract class ElectrumWalletBase extends WalletBase<ElectrumBalance,
  ...
  @override
  DecredWalletKeys get keys => DecredWalletKeys(
      wif: hd.wif!, privateKey: hd.privKey!, publicKey: hd.pubKey!);

  String _password;
  List<DecredUnspent> unspentCoins;
  List<int> _feeRates;
  ...
  @override
  Future<PendingDecredTransaction> createTransaction(
      Object credentials) async {
    const minAmount = 546;
    final transactionCredentials = credentials as DecredTransactionCredentials;
    final inputs = <DecredUnspent>[];
    ...
    if (inputs.isEmpty) {
      throw DecredTransactionNoInputsException();
    }
    ...
    if (hasMultiDestination) {
      if (outputs.any((item) => item.sendAll
        ...
        throw DecredTransactionWrongBalanceException(currency);
      }
    ...
    return PendingDecredTransaction(txb.build(), type,
  ...
  int feeAmountForPriority(DecredTransactionPriority priority, int inputsCount,
          int outputsCount) =>
  ...
  Future<void> updateUnspent() async {
    final unspent = await Future.wait(walletAddresses
        ...
        .then((unspent) => unspent
            .map((unspent) {
              try {
                return DecredUnspent.fromJSON(address, unspent);
              } catch(_) {
                return null;
              }
            }).whereNotNull())));
  ...
  Future<void> _addCoinInfo(DecredUnspent coin) async {
  ...
  Future<Map<String, ElectrumTransactionInfo>> fetchTransactions() async {
    final addressHashes = <String, DecredAddressRecord>{};
