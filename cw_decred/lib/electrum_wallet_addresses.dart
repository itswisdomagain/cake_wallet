...
abstract class ElectrumWalletAddressesBase extends WalletAddresses with Store {
  ElectrumWalletAddressesBase(WalletInfo walletInfo,
      ...
      : addresses = ObservableList<DecredAddressRecord>.of(
          (initialAddresses ?? []).toSet()),
        receiveAddresses = ObservableList<DecredAddressRecord>.of(
          (initialAddresses ?? [])
            .where((addressRecord) => !addressRecord.isHidden && !addressRecord.isUsed)
        .toSet()),
        changeAddresses = ObservableList<DecredAddressRecord>.of(
          (initialAddresses ?? [])
            .where((addressRecord) => addressRecord.isHidden && !addressRecord.isUsed)
        .toSet()),
  ...
  final ObservableList<DecredAddressRecord> addresses;
  final ObservableList<DecredAddressRecord> receiveAddresses;
  final ObservableList<DecredAddressRecord> changeAddresses;
  ...
  Future<void> _discoverAddresses(bitcoin.HDWallet hd, bool isHidden) async {
    var hasAddrUse = true;
    List<DecredAddressRecord> addrs;
  ...
  Future<List<DecredAddressRecord>> _createNewAddresses(int count,
      {required bitcoin.HDWallet hd, int startIndex = 0, bool isHidden = false}) async {
    final list = <DecredAddressRecord>[];
  ...
  void _addAddresses(Iterable<DecredAddressRecord> addresses) {
