...
import 'package:cw_decred/address_from_output.dart';
...
class ElectrumTransactionInfo extends TransactionInfo {
  ...
  factory ElectrumTransactionInfo.fromElectrumVerbose(
      ...
      {required List<DecredAddressRecord> addresses, required int height}) {
    ...
    for (dynamic vin in vins) {
      ...
      inputsAmount +=
          stringDoubleToDecredAmount((out['value'] as double? ?? 0).toString());
    ...
    for (dynamic out in vout) {
      ...
      final value = stringDoubleToDecredAmount(
      ...
      tx.outs.forEach((out) {
        try {
          final p2pkh = bitcoin.P2PKH(
              data: PaymentData(output: out.script), network: decredMainnet);
  ...
  String amountFormatted() =>
      '${formatAmount(decredAmountToString(amount: amount))} ${walletTypeToCryptoCurrency(type).title}';

  @override
  String? feeFormatted() => fee != null
      ? '${formatAmount(decredAmountToString(amount: fee!))} ${walletTypeToCryptoCurrency(type).title}'
      : '';
